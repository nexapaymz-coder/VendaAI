...
export async function generateText(prompt: string) {
  ...
  if (!response.ok) {
    throw new Error(`Erro da IA: ${await response.text()}`);
  }
...
export async function generateImage(prompt: string) {
  ...
  if (!response.ok) {
    throw new Error(`Erro da API de imagem: ${await response.text()}`);
  }
...
export async function generateVideo(image: File, prompt: string) {
  ...
  if (!response.ok) {
    throw new Error(`Erro da API de vídeo: ${await response.text()}`);
  }
